...
def mbm_stat_map(
    ...
    if statTest.lower() == "one sample":
        ...
        for z in range(numVertices):
            ...
            statistic, pValue = scipy.stats.f_oneway(curYY[:, 0], curYY[:, 1])
            statMap[z] = pValue
        ...
        matrixX = numpy.matrix(matrixX)
        # B = (matrixX'*matrixX)\(matrixX'*y);
        B = numpy.matrix(scipy.linalg.solve(matrixX.T * matrixX, matrixX.T * numpy.matrix(inputMap)))
        # eres = y - matrixX*B;
        ...
        C = numpy.matrix(numpy.concatenate((numpy.array([1]), numpy.array([-1]), numpy.zeros(matrixX.shape[1] - 2))))
        ...
        DZero = (D == 0)
        D[DZero] = 1
        statMap = numpy.divide(numpy.multiply(numpy.multiply(G, scipy.linalg.inv(C * scipy.linalg.inv(matrixX.T * matrixX) * C.T)), G), numpy.matrix(D))
        ...
        DZero = (D == 0)
        D[DZero] = 1
        FMap = numpy.divide(numpy.multiply(numpy.multiply(G, scipy.linalg.inv(C * scipy.linalg.inv(matrixX.T * matrixX) * C.T)), G), numpy.matrix(D))
...
def mbm(
    ...
    iNull = scipy.io.matlab.loadmat('../MBM/iNull_emp_two_sample_t.mat')
    ...
    for permutationIDX in range(statNPer):
        ...
        if statTest.lower() == "one sample":
            ...
            #statNullDesignMatrix = numpy.take(statDesignMatrix, iNull, axis=0)
            #statNullDesignMatrix = numpy.take(statDesignMatrix, iNull[:, permutationIDX], axis=0)
            statNullDesignMatrix = numpy.array(statDesignMatrix)
...
if __name__ == "__main__":
    ...
    #     statTest='ANCOVA_Z',
    #     statDesignFile=os.path.join('..', 'MBM', 'data', 'demo_emp', 'G_ANCOVA.txt'),
    #     statNPer=1000,
    #     statPThr=0.1,
    #     statThresh=0.05,
    #     statFDR=False,
